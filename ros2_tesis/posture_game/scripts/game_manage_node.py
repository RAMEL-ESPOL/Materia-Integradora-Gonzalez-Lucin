...
class GameManager(Node):
    def __init__(self):
        ...
        self.POSTURES = [
            ...
            "Ambos puños al frente", "Brazo derecho horizontal", "Brazo izquierdo horizontal", "Ambos brazos horizontales", "Muñeca en nariz"
        ]
        ...
        self.emotion_counts = {"Enojo": 0,"Disgusto": 0,"Miedo": 0,"Felicidad": 0,"Tristeza": 0,"Sorpresa": 0,"Neutral": 0}
        ...
        self.json_publisher = self.create_publisher(String, '/game_stats_json', qos_profile)
        ...
        #Subscripciones
        self.EMOTIONS_LIST = ["Enojo", "Disgusto", "Miedo", "Felicidad", "Tristeza", "Sorpresa", "Neutral"]
    ...
    def play(self):
        ...
        self.declare_parameter('name', 'jugador')  # Valor por defecto
        self.player_name = self.get_parameter('name').get_parameter_value().string_value
        self.get_logger().info(f"Nombre del jugador: {self.player_name}")
...
# ---------------------------------------------------------------------------------------------------------------
            ...
            self.show_message("Ahora tu repite las posturas")
...
#----------------------------------------------------------------------------------------------------------------
    def end_game(self, name):
        ...
        json_data = self.stats_manager.save_stats(
            name=name,
            score=self.score,
            ...
            return_as_dict=True  # <- esto es lo nuevo
        )
        json_str = json.dumps(json_data)
        self.json_publisher.publish(String(data=json_str))
        self.get_logger().info("Estadísticas publicadas en /game_stats_json")
